...
int sensorPin0 = A0;    // Omos 
int sensorPin1 = A1;    // xeri 
int sensorPin2 = A2;    // agkonas  
int sensorPin3 = A3;    // karpos
int sensorPin4 = A4;    //pensa dangana  
...
// arrayMax = max steps we saved to array
...
int SensVal[5]; // sensor value
float dif[5], ist[5], sol[5],  dir[5]; // difference between stored position and momentary position
...
void readPot() // read analog inputs and add some offsets (mechanical corrections)
{
   ...
   SensVal[1] = analogRead(sensorPin1); SensVal[1] += 280; // Omos
   SensVal[2] = analogRead(sensorPin2); SensVal[2] += -50; // heri
   SensVal[3] = analogRead(sensorPin3); // SensVal[3] += 0;// karpos
   SensVal[4] = analogRead(sensorPin4); // SensVal[3] += 0;// gripper
   //Serial.print(SensVal[2]);Serial.print(" "); // CHECK
}
void mapping() // we need microsecond for the servos instead potentiometer values
{
  ist[0] = map(SensVal[0], 150, 900, 600, 2400);//  Omos
  ist[1] = map(SensVal[1], 1000, 100, 550, 2400);// agkonas 
  ist[2] = map(SensVal[2], 120, 860, 400, 2500);// xeri
  ist[3] = map(SensVal[3], 1023, 0, 500, 2500);// karpos
  ist[4] = map(SensVal[4], 1023, 0, 500, 2500);// dangana
...
void calculate()
{
      ...
      if (sol[4] < ist[4]) dir[4] = 0-dif[4]/stepsMax; else dir[4] = dif[4]/stepsMax;
        //Serial.println(dir4); 
...
void data_out() // just to write the recorded data to serial
{
  ...
  while(i < arrayMax)
  {
    digitalWrite(13, HIGH);
    i += 1;
    Serial.print(joint4[i]); Serial.print(", ");
  }
